        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            <el-row v-for="(item1, index1) in scope.row.rightsTree" :key="item1.id" :class="['bottom-border', index1 === 0 ? 'top-border' : '', 'content-center']">
              ...
              <el-col :span="18">
                <el-row v-for="(item2, index2) in item1.children" :key="item2.id" :class="[index2 !== 0 ? 'top-border' : '', 'content-center']">
                  <el-col :span="6">
                    <el-tag type="warning">{{item2.rightsName}}</el-tag>
                  </el-col>
                  <el-col :span="18">
                    <el-tag type="success" v-for="(item3, index3) in item2.children" :key="item3.id">{{item3.rightsName}}</el-tag>
                  </el-col>
                </el-row>
              </el-col>
            </el-row>
    ...
    private addRights() {
      ...
      if(addCheckedKeys.length > 0) {
        createRoleRights({roleId: this.currentRole.id, rightsIds: addCheckedKeys})
          .then((res: any) => {
            if(res.status === 200) {
              this.getRoleList();
              (this as any).$message.success('分配权限成功')
            } else {
              (this as any).$message.error('分配权限失败')
            }
          })
          .catch(err => {
            (this as any).$message.error('分配权限失败')
      ...
      if(removeCheckedKeys.length > 0) {
        destroyRoleRights(this.currentRole.id, {rightsIds: removeCheckedKeys})
          .then((res: any) => {
            if(res.status === 200) {
              (this as any).$message.success('移除权限成功')
            } else {
              (this as any).$message.error('移除权限失败')
            }
          })
          .catch(err => {
            (this as any).$message.error('移除权限失败')
      ...
      getRights({})
        .then((res: any) => {
          if(res.status === 200) {
            ...
            this.rightsArr = res.data.data;
            // console.log(res.data.data);
          } else {
            (this as any).$message.error('获取权限列表失败');
          }
        })
        .catch(err => {
          (this as any).$message.error('获取权限列表失败');
